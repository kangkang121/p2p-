				...
				$("#editForm").validate({
					rules : {
						bidRequestAmount:{
							...
							max:${account.remainBorrowLimitAmount?string('#0.00')}
						},
						currentRate:{
							required:true,
							number:true,
							min:${minRate?string('#0.00')},
							max:${maxRate?string('#0.00')}
						},
						minBidAmount:{
							required:true,
							number:true,
                    ...
                    submitHandler: function(form) {
                        $(form).ajaxSubmit(function (data) {
                            if(data.success){
                                $.messager.confirm("提示","借款申请成功",function () {
	...
	<body>
		...
		<div class="container">
			<div class="el-tip-info">
				<h3>信用借款</h3>
				<p>
					<span class="text-info">可借金额：</span><span class="text-danger">${account.remainBorrowLimitAmount}</span>
				</p>
			</div>
