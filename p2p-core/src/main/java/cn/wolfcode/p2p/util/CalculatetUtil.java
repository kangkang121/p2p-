...
public class CalculatetUtil {
	...
	public static BigDecimal getMonthlyRate(BigDecimal yearRate) {
		...
		return yearRate.divide(ONE_HUNDRED).divide(NUMBER_MONTHS_OF_YEAR, Constants.CAL_SCALE, RoundingMode.HALF_UP);
	}
	...
	public static BigDecimal calTotalInterest(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate,
		...
		if (returnType == Constants.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			// 只借款一个月
			if (monthes2Return == 1) {
				totalInterest = bidRequestAmount.multiply(monthlyRate).setScale(Constants.CAL_SCALE,
				...
				// 算出每月还款
				BigDecimal monthToReturnMoney = temp1.multiply(temp2).divide(temp3, Constants.CAL_SCALE,
						RoundingMode.HALF_UP);
				// 算出总还款
		...
		return DecimalFormatUtil.formatBigDecimal(totalInterest, Constants.STORE_SCALE);
	}
	...
	public static BigDecimal calMonthlyInterest(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate,
		...
		if (returnType == Constants.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			// 只借款一个月
			if (monthes2Return == 1) {
				monthlyInterest = bidRequestAmount.multiply(monthlyRate).setScale(Constants.CAL_SCALE,
						RoundingMode.HALF_UP);
			} else {
				BigDecimal temp1 = bidRequestAmount.multiply(monthlyRate);
				BigDecimal temp2 = (BigDecimal.ONE.add(monthlyRate)).pow(monthes2Return);
				...
				if (monthIndex < monthes2Return) {
					monthlyInterest = DecimalFormatUtil
							.amountFormat((temp1.subtract(monthToReturnMoney)).multiply(temp4).add(monthToReturnMoney)
									.setScale(Constants.CAL_SCALE, RoundingMode.HALF_UP));
	...
	public static BigDecimal calMonthToReturnMoney(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate,
		...
		if (returnType == Constants.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			if (monthes2Return == 1) {
				...
				BigDecimal temp3 = (BigDecimal.ONE.add(monthlyRate)).pow(monthes2Return).subtract(BigDecimal.ONE);
				// 算出每月还款
				monthToReturnMoney = temp1.multiply(temp2).divide(temp3, Constants.CAL_SCALE, RoundingMode.HALF_UP);
			}
		} else if (returnType == Constants.RETURN_TYPE_MONTH_INTEREST) {// 按月到期
			BigDecimal monthlyInterest = bidRequestAmount.multiply(monthlyRate).setScale(Constants.CAL_SCALE,
					RoundingMode.HALF_UP);
			if (monthIndex == monthes2Return) {
				monthToReturnMoney = bidRequestAmount.add(monthlyInterest).setScale(Constants.CAL_SCALE,
						RoundingMode.HALF_UP);
			} else if (monthIndex < monthes2Return) {
				monthToReturnMoney = monthlyInterest;
			}
		}
		return DecimalFormatUtil.formatBigDecimal(monthToReturnMoney, Constants.STORE_SCALE);
	}
	...
	public static BigDecimal calMonthlyReceiveInterest(BigDecimal bidAmount ,BigDecimal bidRequestAmount,BigDecimal psMonthlyInterest){
		return bidAmount.divide(bidRequestAmount,Constants.CAL_SCALE,BigDecimal.ROUND_HALF_UP).multiply(psMonthlyInterest).setScale(Constants.STORE_SCALE,BigDecimal.ROUND_HALF_UP);
	}
	...
	public static BigDecimal calMonthlyReceivePrincipal(BigDecimal bidAmount ,BigDecimal bidRequestAmount,BigDecimal psMonthlyPrincipal){
		return bidAmount.divide(bidRequestAmount,Constants.CAL_SCALE,BigDecimal.ROUND_HALF_UP).multiply(psMonthlyPrincipal).setScale(Constants.STORE_SCALE,BigDecimal.ROUND_HALF_UP);
	}
	...
	public static BigDecimal calBidInterest(BigDecimal bidRequestAmount, int monthes2Return, BigDecimal yearRate,
		...
		// 所占比例
		BigDecimal proportion = acturalBidAmount.divide(bidRequestAmount, Constants.CAL_SCALE, RoundingMode.HALF_UP);
		BigDecimal bidInterest = totalInterest.multiply(proportion);
		return DecimalFormatUtil.formatBigDecimal(bidInterest, Constants.STORE_SCALE);
	}
	...
	public static BigDecimal calAccountManagementCharge(BigDecimal bidRequestAmount) {
		BigDecimal accountManagementCharge = DecimalFormatUtil
				.formatBigDecimal(bidRequestAmount.multiply(ACCOUNT_MANAGER_CHARGE_RATE), Constants.STORE_SCALE);
